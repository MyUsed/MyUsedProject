...
public class WriteController {
	...
	@RequestMapping("/mainSubmit1.nhn")
	public String mainSubmit(MultipartHttpServletRequest request , MainboardDTO dto ,MainProboardDTO prodto , String sendPay ,String deposit){
		...
		int num = (int) sqlMap.queryForObject("main.num", sessionId); // 접속한
																		// 아이디로
																		// num
																		// 가져오기
		String name = (String) sqlMap.queryForObject("main.name", sessionId); // 접속한
																				// 아이디로
																				// 이름
																				// 가져오기
		...
		if (Submit.equals("update")) { // submit이 update일때 실행
			...
			if (request.getFile("image1").isEmpty()) {
				...
				for (int i = 1; i <= 8; i++) {
					...
					if (!mf.isEmpty()) { // mf에 파일이 담겼는지 확인 한후 있으면 업로드 수행
						...
						File copy = new File(
								"E:\\Jsp Example\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp0\\wtpwebapps\\MyUsed\\images\\"
										+ orgName); // 업로드
			...
			String SendPay = request.getParameter("sendPay"); // 배송료 포함인지 아닌지
																// 받아옴
			...
			if (request.getFile("pimage1").isEmpty()) { // 사진 선택을 하지 않으면
				...
				for (int i = 1; i <= 8; i++) {
					MultipartFile mf = request.getFile("pimage" + i); // 파일을 받는
																		// MultipartFile
																		// 클래스
																		// (원본)
					String orgName = mf.getOriginalFilename();
		...
		int boardnums = (int) sqlMap.queryForObject("main.boardnum", null); // 접속한
																			// 아이디로
																			// num
																			// 가져오기
