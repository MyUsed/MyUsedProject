...
<h3>QnA</h3>
	...
	<c:if test="${count4 != 0}">
				...
				<c:forEach var="list4" items="${list4}" varStatus="i">
					<table border="1" width="500" height="20">
					<tr>
						<td width="20">${list4.seq_num}</td>
						<td width="130">${list4.title}</td>
						<td width="50">${list4.readcount}</td>
						<td width="50">${list4.mem_id}</td>
						<td width="50"><c:if test="${fn:length(list4.reg) > 11}">
								${fn:substring(list4.reg,0,10)}
							</c:if>
						</td>
					</tr>
					</table>
